...
3. We provided `policy.device=cuda` since we are training on a Nvidia GPU, but you could use `policy.device=xpu` for Intel GPU, `policy.device=mps` to train on Apple silicon, or `policy.device=cpu` for CPU-only training.
